...
../configure --prefix=/usr --host=$HOST --disable-werror && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"

# bash
unpack "bash-5.3.tar.gz" "bash-5.3" "tar.gz"
cd "$BUILD_DIR/bash-5.3"
./configure --prefix=/usr --host=$HOST --enable-readline --without-bash-malloc && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"

# coreutils
unpack "coreutils-9.11.tar.xz" "coreutils-9.11" "tar.xz"
cd "$BUILD_DIR/coreutils-9.11"
./configure --prefix=/usr --host=$HOST && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"

# openssl
unpack "openssl-4.0.1.tar.gz" "openssl-4.0.1" "tar.gz"
cd "$BUILD_DIR/openssl-4.0.1"
./Configure "$OPENSSL_TARGET" --prefix=/usr --cross-compile-prefix="$CROSS_COMPILE" no-shared && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"

# libarchive
unpack "libarchive-3.8.8.tar.xz" "libarchive-3.8.8" "tar.xz"
cd "$BUILD_DIR/libarchive-3.8.8"
./configure --prefix=/usr --host=$HOST --disable-shared && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"

# curl
unpack "curl-8.21.0.tar.xz" "curl-8.21.0" "tar.xz"
cd "$BUILD_DIR/curl-8.21.0"
./configure --prefix=/usr --host=$HOST --with-openssl --disable-shared && make -s -j$JOBS && make install DESTDIR="$SYSROOT"
cd "$OLDPWD"
...
meson setup build --buildtype plain --prefix=/usr -Dpkg-ext=.pkg.tar.zst && meson compile -C build && DESTDIR="$SYSROOT" meson install -C build
cd "$OLDPWD"
